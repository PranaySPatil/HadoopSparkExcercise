...
df = flight_data_date.filter(flight_data_date.day_of_month == 14).filter(flight_data_date.month == 3).groupBy(flight_data_date.carrier).agg({"arr_delay":"sum"})
...
#Q4
flight_data_denorm = spark.table("flights.flight_data_denorm")
df=flight_data_denorm.filter(flight_data_denorm.DAY_OF_MONTH == 14).filter(flight_data_denorm.MONTH == 3).groupBy(flight_data_denorm.CARRIER_NAME).agg({"ARR_DELAY":"sum"})
df=df.withColumnRenamed("sum(ARR_DELAY)","ARR_DELAY")
